...
def filter_inters(
    inters, can_items=None, user_k_core_threshold=0, item_k_core_threshold=0
):
    new_inters = []
...
def generate_item_embedding(
    args, item_text_list, item2index, tokenizer, model, word_drop_ratio=-1
):
    ...
    while start < len(order_texts):
        ...
        if args.emb_type == "CLS":
            ...
            mean_output = masked_output[:, 1:, :].sum(dim=1) / encoded_sentences[
                "attention_mask"
...
if __name__ == "__main__":
    ...
    write_text_file(
        item_text_list,
        os.path.join(args.output_path, args.dataset, f"{args.dataset}.text"),
    )
    write_remap_index(
        user2index,
        os.path.join(args.output_path, args.dataset, f"{args.dataset}.user2index"),
    )
    write_remap_index(
        item2index,
        os.path.join(args.output_path, args.dataset, f"{args.dataset}.item2index"),
    )
