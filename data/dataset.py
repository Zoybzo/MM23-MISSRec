...
class MISSRecDataset(SequentialDataset):
    def __init__(self, config):
        ...
        self.img_size = (
            config["img_size"] if hasattr(config, "img_size") else config["plm_size"]
        )
        ...
        if self.config["img_suffix"] is not None:
    ...
    def load_plm_embedding(self):
        feat_path = osp.join(
            self.config["data_path"], f"{self.dataset_name}.{self.plm_suffix}"
        )
    ...
    def load_img_embedding(self):
        feat_path = osp.join(
            self.config["data_path"], f"{self.dataset_name}.{self.img_suffix}"
        )
        loaded_feat = np.fromfile(feat_path, dtype=np.float32).reshape(
            -1, self.img_size
        )
...
class PretrainMISSRecDataset(MISSRecDataset):
    def __init__(self, config):
        ...
        plm_embedding_weight_aug = self.load_plm_embedding(
            plm_suffix_aug=self.plm_suffix_aug
        )
        ...
        self.plm_embedding_empty_mask = self.get_embedding_empty_mask(
            self.plm_embedding
        )
        self.plm_interest_lookup_table = IndexableBuffer(
            torch.zeros(self.plm_embedding.num_embeddings, dtype=torch.long)
        )
        # 改进3：引入mask，说明哪里是空的哪里不是
        self.img_embedding_empty_mask = self.get_embedding_empty_mask(
            self.img_embedding
        )
        self.img_interest_lookup_table = IndexableBuffer(
            torch.zeros(self.img_embedding.num_embeddings, dtype=torch.long)
        )

        # NOTE: 只在下游微调时起效
        all_num_embeddings = (
            self.plm_embedding.num_embeddings + self.img_embedding.num_embeddings - 2
        )
        ...
        self.interest_embeddings = IndexableBuffer(
    ...
    def load_plm_embedding(self, plm_suffix_aug=None):
        with open(
            osp.join(self.config["data_path"], f"{self.dataset_name}.pt_datasets"), "r"
        ) as file:
        ...
        for dataset_name in dataset_names:
            if plm_suffix_aug is None:
                feat_path = osp.join(
                    self.config["data_path"], f"{dataset_name}.{self.plm_suffix}"
                )
            else:
                feat_path = osp.join(
                    self.config["data_path"], f"{dataset_name}.{plm_suffix_aug}"
                )
            loaded_feat = np.fromfile(feat_path, dtype=np.float32).reshape(
                -1, self.plm_size
            )
    ...
    def load_img_embedding(self, plm_suffix_aug=None):
        ...
        for dataset_name in dataset_names:
            feat_path = osp.join(
                self.config["data_path"], f"{dataset_name}.{self.img_suffix}"
            )
            loaded_feat = np.fromfile(feat_path, dtype=np.float32).reshape(
                -1, self.plm_size
            )
