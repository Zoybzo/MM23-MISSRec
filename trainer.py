...
class MISSRecTrainer(Trainer):
    ...
    def _multimodal_interest_discovery(self, epoch_idx=None):
        self.logger.info(
            f"Discovering multi-modal user interest"
            + ("" if epoch_idx is None else f" before {epoch_idx}-th epoch")
        )
        all_embedding_list = []
        self.model.eval()
        ...
        if "img" in self.model.modal_type:
            all_img_embeddings = self.model.img_adaptor(
                self.model.img_embedding.weight.data[1:]
            )
        ...
        read_ptr = 0
        if "text" in self.model.modal_type:
            self.model.plm_interest_lookup_table[1 : 1 + len(all_text_embeddings)] = (
                cluster_idx[read_ptr : read_ptr + len(all_text_embeddings)]
            )
            read_ptr += len(all_text_embeddings)
        if "img" in self.model.modal_type:
            self.model.img_interest_lookup_table[1 : 1 + len(all_img_embeddings)] = (
                cluster_idx[read_ptr : read_ptr + len(all_img_embeddings)]
            )
            read_ptr += len(all_img_embeddings)
        assert read_ptr == len(
            cluster_idx
        ), f"read_ptr={read_ptr}, len(cluster_idx)={len(cluster_idx)}"
        del all_embedding_list, all_embedding, cluster_idx, centroids
        self.logger.info(
            f"Finish multi-modal interest discovery"
            + ("" if epoch_idx is None else f" before {epoch_idx}-th epoch")
        )
...
class DDPPretrainTrainer(Trainer):
    def __init__(self, config, model):
        ...
        self.logger.info(
            f'Let\'s use {torch.cuda.device_count()} GPUs to train {self.config["model"]} ...'
        )
            ...
            if (epoch_idx + 1) % self.save_step == 0 and self.lrank == 0:
                saved_model_file = os.path.join(
                    ...
                    "{}-{}-{}-{}.pth".format(
                        ...
                        str(epoch_idx + 1),
                        (
                            self.config["ckpt_suffix"]
                            if "ckpt_suffix" in self.config
                            else "paper"
                        ),
                    ),
                )
    ...
    def _multimodal_interest_discovery(self, epoch_idx=None):
        ...
        if self.rank == 0:
            ...
            cluster_idx = torch.zeros(
                cluster_idx_num, dtype=torch.long, device=self.model.device
            )
        ...
        read_ptr = 0
        if "text" in self.model.modal_type:
            self.model.plm_interest_lookup_table[1 : 1 + text_embedding_num] = (
                cluster_idx[read_ptr : read_ptr + text_embedding_num]
            )
            read_ptr += text_embedding_num
        if "img" in self.model.modal_type:
            self.model.img_interest_lookup_table[1 : 1 + img_embedding_num] = (
                cluster_idx[read_ptr : read_ptr + img_embedding_num]
            )
...
class DDPMISSRecPretrainTrainer(DDPPretrainTrainer):
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        Returns:
            ...
            tuple which includes the sum of loss in each part.
        """
        self._multimodal_interest_discovery(
            train_data, epoch_idx, preproc_bs=train_data.batch_size
        )
        self.model.train()
        loss_func = loss_func or self.model.calculate_loss
        total_loss = None
    ...
    @torch.no_grad()
    def _multimodal_interest_discovery(
        self, dataloader, epoch_idx=None, preproc_bs=512
    ):
        self.logger.info(
            f"Discovering multi-modal user interest"
            + ("" if epoch_idx is None else f" before {epoch_idx}-th epoch")
        ...
        if self.rank == 0:
            ...
            if "text" in self.model.modal_type:
                ...
                for ptr in range(1, len(plm_embedding_data), preproc_bs):
                    batch_data = plm_embedding_data[ptr : ptr + preproc_bs].to(
                        self.model.device
                    )
                    text_embeddings = self.model.text_adaptor(batch_data)
                    del batch_data
                    all_text_embeddings.append(
                        text_embeddings.to(plm_embedding_data.device)
                    )
                    del text_embeddings
            ...
            if "img" in self.model.modal_type:
                ...
                for ptr in range(1, len(img_embedding_data), preproc_bs):
                    batch_data = img_embedding_data[ptr : ptr + preproc_bs].to(
                        self.model.device
                    )
                    img_embeddings = self.model.img_adaptor(batch_data)
                    del batch_data
                    all_img_embeddings.append(
                        img_embeddings.to(img_embedding_data.device)
                    )
                    del img_embeddings
            ...
            cluster_idx = torch.zeros(
                cluster_idx_num,
                dtype=torch.long,
                device=dataloader.dataset.interest_embeddings.device,
            )
        ...
        if "text" in self.model.modal_type:
            if self.model.train_stage == "pretrain":
                dataloader.dataset.plm_interest_lookup_table[
                    1 : 1 + text_embedding_num
                ] = cluster_idx[read_ptr : read_ptr + text_embedding_num]
            else:
                dataloader.dataset.plm_interest_lookup_table[
                    1 : 1 + text_embedding_num
            ...
            read_ptr += text_embedding_num
        if "img" in self.model.modal_type:
            dataloader.dataset.img_interest_lookup_table[1 : 1 + img_embedding_num] = (
                cluster_idx[read_ptr : read_ptr + img_embedding_num]
            )
            read_ptr += img_embedding_num
        assert read_ptr == len(
